...
public OnGameModeInit() {
	...
	if (MMDB_Load("GeoLite2-City.mmdb")) {
		...
		print("[MMDB] Failed to load GeoLite2-City.mmdb.");
	}
...
public OnPlayerConnect(playerid) {
	...
	if (MMDB_GetCountryName(ip, country, sizeof(country), MMDB_LANG_ENGLISH)) {
		...
		printf("[MMDB] Player %d connected | IP: %s | Country: Unknown", playerid, ip);
	}
...
*This project relies on [libmaxminddb](https://github.com/maxmind/libmaxminddb) and the modern [open.mp SDK](https://github.com/openmultiplayer/open.mp).*
