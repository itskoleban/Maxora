 ...
 *
 * This file is used to test libmaxminddb integration without needing a running SA-MP / open.mp server.
 * It compiles to a standard terminal executable (`maxora_test.exe`) and attempts to read a database
 * file to verify the behavior of the API.
 */
...
int main()
{
    ...
    if (status != MMDB_SUCCESS) {
        ...
        std::cerr << "Make sure 'GeoLite2-Country.mmdb' is in the same directory as the executable.\n";
        return 1;
    }
        ...
        for (const auto& path : test_paths) {
            ...
            if (aget_status == MMDB_SUCCESS && entry_data.has_data) {
                if (entry_data.type == MMDB_DATA_TYPE_UTF8_STRING) {
                    // Strings in libmaxminddb are not null-terminated, so we must construct std::string with data_size
